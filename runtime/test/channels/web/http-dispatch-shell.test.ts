...
describe("web http shell dispatch", () => {
  ...
  test("dispatches index/manifest/service-worker/static/docs/sse/terminal-session/vnc routes", async () => {
    const channel = {
      ...
      handleLspSession: () => new Response("lsp-session"),
      handleLspHandoff: async () => new Response("lsp-handoff"),
      handleLspGetSettings: () => new Response("lsp-settings"),
      handleLspUpdateSettings: async () => new Response("lsp-update-settings"),
    ...
    expect(await (await handleShellRoutes(channel, new Request("https://e/lsp/session?path=src/app.ts", { method: "GET" }), "/lsp/session", buildRouteFlags(), async () => new Response()))?.text()).toBe("lsp-session");
    expect(await (await handleShellRoutes(channel, new Request("https://e/lsp/handoff?path=src/app.ts", { method: "POST" }), "/lsp/handoff", buildRouteFlags(), async () => new Response()))?.text()).toBe("lsp-handoff");
    expect(await (await handleShellRoutes(channel, new Request("https://e/lsp/settings", { method: "GET" }), "/lsp/settings", buildRouteFlags(), async () => new Response()))?.text()).toBe("lsp-settings");
    expect(await (await handleShellRoutes(channel, new Request("https://e/lsp/settings", { method: "POST" }), "/lsp/settings", buildRouteFlags(), async () => new Response()))?.text()).toBe("lsp-update-settings");
