...
export function upsertProxmoxConfig(
  config: Omit<ProxmoxConfig, "created_at" | "updated_at">,
): ProxmoxConfig {
  const db = getDb();
  const now = new Date().toISOString();
  db.prepare(
    `INSERT INTO proxmox_configs (
...
export function deleteProxmoxConfig(chatJid: string): boolean {
  const db = getDb();
  const result = db.prepare("DELETE FROM proxmox_configs WHERE chat_jid = ?").run(chatJid);
  return result.changes > 0;
}

export function listProxmoxConfigs(): ProxmoxConfig[] {
  const db = getDb();
  const rows = db.prepare(
    `SELECT chat_jid, base_url, api_token_keychain, allow_insecure_tls, created_at, updated_at
       FROM proxmox_configs
      ORDER BY updated_at DESC, chat_jid ASC`
  ).all() as ProxmoxConfig[];
  return rows.map((row) => ({
    ...row,
    allow_insecure_tls: normalizeBoolean(row.allow_insecure_tls),
  }));
