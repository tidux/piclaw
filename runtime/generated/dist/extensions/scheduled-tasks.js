...
function normalizeChatJid(chatJid) {
...
function failureDetails(action, extra = {}) {
    return {
        action,
        ok: false,
        confirmed: false,
...
function successDetails(action, extra = {}) {
    return {
        action,
        ok: true,
        confirmed: true,
...
function asTaskKind(task) {
    if (!task)
        return null;
    return (task.task_kind ?? "agent");
}
function makeTextResult(text, details) {
    return {
        content: [{ type: "text", text }],
        details,
    };
}
function createScheduledTask(params) {
    ...
    if (taskKind === "agent") {
        ...
        return makeTextResult(`Scheduled agent task for ${chatJid}.`, successDetails("create", {
            id: taskId,
            task_kind: "agent",
            next_run: nextRun,
            chat_jid: chatJid,
    ...
    if (params.model) {
        return makeTextResult("Model overrides are not supported for shell tasks.", failureDetails("create", { chat_jid: chatJid, task_kind: "shell" }));
    }
    ...
    return makeTextResult(`Scheduled shell task for ${chatJid}.`, successDetails("create", {
        id: taskId,
        task_kind: "shell",
        next_run: nextRun,
        chat_jid: chatJid,
...
function getTaskForMutation(action, params) {
    ...
    if ((task.task_kind ?? "agent") === "internal" && !allowInternal) {
        return makeTextResult(`Task ${id} is internal and cannot be ${action}d unless allow_internal=true.`, failureDetails(action, {
            found: true,
            id,
            protected: true,
...
function pauseTask(params) {
    const loaded = getTaskForMutation("pause", params);
    if ("content" in loaded)
        return loaded;
    const { task, id } = loaded;
    if (task.status === "paused") {
        return makeTextResult(`Task ${id} is already paused.`, successDetails("pause", {
            id,
            found: true,
            noop: true,
            task_kind: asTaskKind(task),
    ...
    if (task.status === "completed") {
        return makeTextResult(`Task ${id} is completed and cannot be paused.`, failureDetails("pause", {
            found: true,
            id,
            task_kind: asTaskKind(task),
            old_status: task.status,
    ...
    return makeTextResult(`Paused task ${id}.`, successDetails("pause", {
        id,
        found: true,
...
function resumeTask(params) {
    const loaded = getTaskForMutation("resume", params);
    if ("content" in loaded)
        return loaded;
    const { task, id } = loaded;
    if (task.status === "active") {
        return makeTextResult(`Task ${id} is already active.`, successDetails("resume", {
            id,
            found: true,
            noop: true,
            task_kind: asTaskKind(task),
    ...
    if (!task.next_run) {
        return makeTextResult(`Task ${id} has no next_run and cannot be resumed.`, failureDetails("resume", {
            found: true,
            id,
            task_kind: asTaskKind(task),
            old_status: task.status,
    ...
    const updated = getTaskById(id);
    return makeTextResult(`Resumed task ${id}.`, successDetails("resume", {
        id,
        found: true,
        task_kind: asTaskKind(updated),
        old_status: task.status,
...
function deleteScheduledTask(params) {
    const loaded = getTaskForMutation("delete", params);
    if ("content" in loaded)
        return loaded;
    const { task, id } = loaded;
    deleteTask(id);
    return makeTextResult(`Deleted task ${id}.`, successDetails("delete", {
        id,
        found: true,
        deleted: true,
        task_kind: asTaskKind(task),
...
function inspectScheduledTasks(params) {
...
function executeScheduledTasks(params) {
    const action = params.action || "list";
    switch (action) {
        case "create":
...
export const scheduledTasks = (pi) => {
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            return executeScheduledTasks(params);
        },
    });
    pi.registerTool({
        name: "schedule_task",
        ...
        async execute(_toolCallId, params) {
            return executeScheduledTasks({ ...params, action: "create" });
        },
    });
};
