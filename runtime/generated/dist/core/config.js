...
export const STORE_DIR = resolve(CLI_WORKSPACE ? `${WORKSPACE_DIR}/.piclaw/store` : (process.env.PICLAW_STORE || `${WORKSPACE_DIR}/.piclaw/store`));
/** Directory for runtime data (sessions, IPC files, etc.). */
export const DATA_DIR = resolve(CLI_WORKSPACE ? `${WORKSPACE_DIR}/.piclaw/data` : (process.env.PICLAW_DATA || `${WORKSPACE_DIR}/.piclaw/data`));
// ---------------------------------------------------------------------------
...
export function isDefaultWebTerminalEnabled(platform = process.platform) {
    return platform === "linux" || platform === "darwin";
}
export function isDefaultWebVncDirectEnabled(platform = process.platform) {
...
export const WEB_RUNTIME_CONFIG = Object.seal({
    ...
    vncTargetsRaw: process.env.PICLAW_WEB_VNC_TARGETS ||
...
const sessionMaxLines = pickNumber({ PICLAW_SESSION_MAX_LINES: process.env.PICLAW_SESSION_MAX_LINES ?? envConfig.PICLAW_SESSION_MAX_LINES }, [
...
const workspaceSearchExtensions = pickStringArray({ PICLAW_WORKSPACE_SEARCH_EXTENSIONS: process.env.PICLAW_WORKSPACE_SEARCH_EXTENSIONS ?? envConfig.PICLAW_WORKSPACE_SEARCH_EXTENSIONS }, ["PICLAW_WORKSPACE_SEARCH_EXTENSIONS"]) ?? configWorkspaceSearchExtensions ?? [];
