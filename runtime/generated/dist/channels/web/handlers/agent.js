 ...
 */
import { getAgentRuntimeConfig, getIdentityConfig, getRoutingConfig, } from "../../../core/config.js";
...
function isModelConfigError(errorText) {
    if (!errorText)
        return false;
...
function formatUserVisibleError(errorText, rateLimited) {
    if (rateLimited) {
...
export function buildRecoveryMarkerBlocks(recovery) {
    if (!recovery?.attemptsUsed)
        return undefined;
    return [{
...
export async function handleAgentMessage(channel, req, pathname, chatJid, defaultAgentId) {
    ...
    if (metersCommand) {
        ...
        return channel.json({ thread_id: null, command: metersCommand, ui_only: true }, 200);
    }
    ...
    const queueSteerMessage = async (source) => {
        ...
        if (rootRowId && rootRowId !== interaction.id) {
            ...
            broadcastInteractionUpdated(channel, interaction, currentIdentity.assistantName, resolveAvatarUrl("agent", currentIdentity.assistantAvatar), currentIdentity.userName || null, resolveAvatarUrl("user", currentIdentity.userAvatar), currentIdentity.userAvatarBackground || null);
        }
