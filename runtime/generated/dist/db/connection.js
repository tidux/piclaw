...
function ensureWebSessionColumns(database) {
    const columns = database.prepare("PRAGMA table_info(web_sessions)").all();
    const existing = new Set(columns.map((col) => col.name));
 ...
 */
function dropChatBranchDisplayName(database) {
    const cols = database.prepare("PRAGMA table_info(chat_branches)").all();
...
export function closeDatabase(options) {
    if (!db)
        return;
    const database = db;
    db = null;
