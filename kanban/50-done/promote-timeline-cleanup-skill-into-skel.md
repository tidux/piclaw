---
id: promote-timeline-cleanup-skill-into-skel
title: Promote timeline-cleanup skill into skel
status: done
priority: medium
created: 2026-03-17
updated: 2026-03-17
...
# Promote timeline-cleanup skill into skel

## Summary

The private `timeline-cleanup` skill was triaged as a good candidate to move
...
- `timeline-cleanup` exists under `skel/.pi/skills/`.
...
1. Copy `/.pi/skills/timeline-cleanup/` into `skel/.pi/skills/timeline-cleanup/`.
...
- Verify the skill files exist under `skel/.pi/skills/timeline-cleanup/`.
...
- Lane change: `10-next` → `50-done` after copying `timeline-cleanup` into `skel/.pi/skills/timeline-cleanup/`.
- Verified the skel copy includes both `SKILL.md` and `cleanup.ts`.
- Validation:
  - `bun run skel/.pi/skills/timeline-cleanup/cleanup.ts --help`
- This skill remained self-contained and did not require extra dependency seeding beyond its own directory.
